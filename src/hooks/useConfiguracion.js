...
export function usePromociones() {
  ...
  useEffect(() => {
    ...
    promocionesService.getPromociones()
      .then(res => { if (ignore) return; const data = Array.isArray(res.data) ? res.data : []; const norm = data.map(p => ({ ...p, id_promo: p.id_promo ?? p.id, precio_combo: Number(p.precio_combo) || 0 })); setPromociones(norm); saveCache(PROMO_KEY, norm); setError(null); })
  ...
  const recargar = useCallback(async () => {
    ...
    try {
      ...
      const norm = data.map(p => ({ ...p, id_promo: p.id_promo ?? p.id, precio_combo: Number(p.precio_combo) || 0 }));
